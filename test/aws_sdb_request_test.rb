...
class AwsSdbRequestTest < Test::Unit::TestCase
  ...
  should "url encode the param values" do
    ...
    assert_match /#{safe_chars}/, req.send(:params_query)
    assert_match /%2B%2A%21%40%C2%A3%24%25%5E%26%2A%28%29/, req.send(:params_query)
  end
  ...
  should "add the actual signature" do
    req = Base.new('GET', {'a' => '1'}, {:account => 'account', :secret => 'secret'})
    assert_match /&Signature=[0-9a-zA-Z%]+$/, req.uri_query
  end
